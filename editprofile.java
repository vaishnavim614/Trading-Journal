   ...
   public void profile(){
      ...
      //  Bytes photo = timage.getBytes();
        ...
        try{
            ...
            PreparedStatement pst = con.prepareStatement("insert into edit_profile(name,email,dob,mobile,address,gender,photo)values(?,?,?,?,?,?,?)");
...
public boolean updateprofile(){
    ...
    boolean isupdated = false;
        ...
        try{
            Connection con = DBConnection.getConnection();
            PreparedStatement pst = con.prepareStatement("update edit_profile set name = ? , email = ? ,dob = ? ,mobile = ? , address = ? , gender = ? ,photo = ? where user_id = '1'");
  ...
  //  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
   // String dob = sdf.format(tdob.getDate());
    ...
    try{
            ...
            rs = st.executeQuery("select name from edit_profile");
            rs.next();
            ...
            rs = st.executeQuery("select email from edit_profile");
            rs.next();
            temail.setText(rs.getString(1));
            
            rs = st.executeQuery("select dob from edit_profile");
            rs.next();
            tdob.setDate(rs.getDate(1));
            
            rs = st.executeQuery("select mobile from edit_profile");
            rs.next();
            tmobile.setText(rs.getString(1));
            
            rs = st.executeQuery("select address from edit_profile");
            rs.next();
            taddress.setText(rs.getString(1));
            
            rs = st.executeQuery("select gender from edit_profile");
            rs.next();
            tgender.setSelectedItem(rs.getString(1));
            
            rs = st.executeQuery("select photo from edit_profile");
            rs.next();
